import { campaignApprovalRequestSchema, campaignCreateRequestSchema, campaignSubmitRequestSchema, campaignUpdateRequestSchema } from "@wa-blast/contracts";
...
import { ApprovalService, approvalActorFromAuth } from "./approval-service";

export function registerCampaignRoutes(app: Hono<AuthEnv>, dependencies: { service: CampaignService; approval: ApprovalService; auth: AuthService; config: AppConfig }): void {
  const { service, approval, auth, config } = dependencies;
  ...
  app.post("/api/campaigns/:campaignId/approve", user, csrf, requireRole("admin"), recent, async (context) => {
    const parsed = campaignApprovalRequestSchema.safeParse(await readJson(context));
    if (!parsed.success) return context.json({ error: "invalid_request" }, 400);
    try { return context.json(await approvalResponse(approval.approve(context.req.param("campaignId"), parsed.data.version, approvalActorFromAuth(context.get("auth"))))); } catch (error) { return campaignError(context, error); }
  });

  app.post("/api/campaigns/:campaignId/reopen", user, csrf, async (context) => {
    try { return context.json({ campaign: approval.reopen(context.req.param("campaignId"), campaignActorFromAuth(context.get("auth"))) }); } catch (error) { return campaignError(context, error); }
  });
...
function campaignError(context: { json: (data: unknown, status?: number) => Response }, error: unknown): Response {
  ...
  const conflictCodes = new Set(["version_conflict", "campaign_not_editable", "campaign_not_pending_approval", "campaign_not_reopenable", "campaign_jobs_claimed"]);
...
async function approvalResponse(result: ReturnType<ApprovalService["approve"]>) { return result; }
